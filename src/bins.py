...
class BinPool:
    ...
    def _place_not_covered(self, singles_removed=None, area_tol=1.0):
        ...
        def _get_new_layer(to_place):
            ...
            spool = utils.SuperitemPool(superitems=to_place)
            layer = utils.maxrects_single_layer_online(spool, self.pallet_dims)
            return layer
        ...
        if len(remaining_items) > 0:
            spool = utils.SuperitemPool(superitems=remaining_items)
            lpool = utils.maxrects_multiple_layers(spool, self.pallet_dims, add_single=False)
