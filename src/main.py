...
import utils, config, column_generation, baseline, bins
...
def get_height_groups(superitems_pool, pallet_dims, height_tol=0, density_tol=0.5):
    ...
    for height in unique_heights:
        ...
        spool = utils.SuperitemPool(superitems=spool)
        if (
...
def maxrects_warm_start(superitems_pool, height_tol=0, density_tol=0.5, add_single=True):
    ...
    mr_layer_pool = utils.LayerPool(superitems_pool, config.PALLET_DIMS)
    ...
    for i, spool in enumerate(height_groups):
        ...
        layer_pool = utils.maxrects_multiple_layers(
...
def cg(
    ...
    cg_layer_pool = utils.LayerPool(superitems_pool, config.PALLET_DIMS)
    ...
    for i, spool in enumerate(height_groups):
        ...
        if mr_warm_start:
            ...
            warm_start_layer_pool = utils.LayerPool(spool, config.PALLET_DIMS, add_single=True)
...
def main(
    ...
    final_layer_pool = utils.LayerPool(utils.SuperitemPool(), config.PALLET_DIMS)
    ...
    for iter in range(max_iters):
        ...
        superitems_list, singles_removed = utils.SuperitemPool.gen_superitems(
            order=working_order,
        ...
        superitems_pool = utils.SuperitemPool(superitems=superitems_list)
